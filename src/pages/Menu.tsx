...
const Menu = () => {
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h1 className="text-3xl font-bold mb-4 text-red-500">brunch</h1>
      {MenuList.brunch.map((section, index) => (
        <div key={index}>
          <p className="italic text-gray-600 mb-2">{section.times}</p>

          <h2 className="text-2xl font-semibold mt-5 text-red-500">starters</h2>
          {section.starters.map((item, idx) => (
            <MenuItemCard key={idx} item={item} />
          ))}

          <h2 className="text-2xl font-semibold mt-5 text-red-500">salads</h2>
          {section.salads.map((item, idx) => (
            <MenuItemCard key={idx} item={item} />
          ))}

          <h2 className="text-2xl font-semibold mt-5 text-red-500">mains</h2>
          {section.mains.map((item, idx) => (
            <MenuItemCard key={idx} item={item} />
          ))}

          <h2 className="text-2xl font-semibold mt-5 text-red-500">sides</h2>
      ...
      <h1 className="text-3xl font-bold mt-8 text-red-500">pastries</h1>
      {MenuList.pastries.map((item, idx) => (
        <MenuItemCard key={idx} item={item} />
      ))}

      <h1 className="text-3xl font-bold mt-8 text-red-500">drinks</h1>
      {MenuList.drinks.map((item, idx) => (
        <MenuItemCard key={idx} item={item} />
      ))}
      
      <h1 className="text-3xl font-bold mt-8 text-red-500">weekly specials</h1>
...
const MenuItemCard = ({ item }: { item: MenuItem }) => (
  <div className="pt-2">
    <h3 className="text-lg font-medium text-blue-500 mt-2">{item.name} - ${item.price}</h3>
